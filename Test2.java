...
public class AMCRedlineReportSA {// implements IEventAction{

	private static final Logger pxLogger = LogManager.getLogger(AMCRedlineReportSA.class);
	...
	ArrayList<IItem> newItemList = null;

	IAgileSession adminSession;

	String pxName = "PX AMCRedlineReport Ver ";
	...
	public ActionResult doAction(IAgileSession session, String x, IChange amcObj) {
		...
		try {
			...
			newItemList = new ArrayList<IItem>();

			// objChange = (IChange) eventInfo.getDataObject();

			/*
			 * 
			 * 
			 * adminSession = repUtil.createAdminSession(); formattedString =
			 * String.format("Re-loading objects with Admin Session: %s" ,
			 * adminSession.getCurrentUser()); pxLogger.info(formattedString);
			 * objChange = (IChange) adminSession.getObject(IChange.OBJECT_TYPE,
			 * objChange.toString()); formattedString =
			 * String.format("Re-loaded objects with Admin Session: %s" ,
			 * objChange); pxLogger.info(formattedString);
			 */

			String strStatus = amcObj.getValue(ChangeConstants.ATT_COVER_PAGE_STATUS).toString();
			formattedString = String.format("strStatus:%s", strStatus);
			pxLogger.info(formattedString);
			...
			System.out.println("Revised flag:" + revisedFlag);
			...
			if (!affItemTable.isEmpty() && !validItemList.isEmpty()) {
				...
				// To attach the Redline Report to AMC Attachments tab
				...
				System.out.println(constObj.noAffItems);
			}
			...
			// return new EventActionResult(request, new
			// ActionResult(ActionResult.EXCEPTION, new Exception(strMessage)));
			pxLogger.info("Exception occured%s", e.getMessage());
		} finally {
		...
		pxLogger.info("final action result%s", actionResult);
	...
	public static void main(String args[]) {
		...
		AMCRedlineReportSA g = new AMCRedlineReportSA();
		IAgileSession session = g.createAgileSession();
		pxLogger.info("session created" + session);
		try {
			IChange objChange = (IChange) session.getObject(IChange.OBJECT_TYPE, "AMC-019609"); // 019509
			// //019508 AMC-001688
			// g.a(session, amc);24685
			System.out.println("in main");
			g.doAction(session, null, objChange);
		} catch (Exception e) {
	...
	public IAgileSession createAgileSession() {
		IAgileSession session = null;
		try {

			HashMap<Integer, String> params = new HashMap<>();
			AgileSessionFactory factory = AgileSessionFactory
					.getInstance("http://ame-agl-dap11.arthrex.local:7001/Agile/");

			params.put(AgileSessionFactory.PX_USERNAME, "integration_user");
			params.put(AgileSessionFactory.PX_PASSWORD, "agile1234");
